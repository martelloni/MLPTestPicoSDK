...
- **Measured, not negligible: this WAS the 1.7x slowdown.** Tagging `StaticLayer`'s hot path with `SMLP_CODE_ATTR` (`MEML_RUNS_ON_CORE_CODE`, which forced `noinline`/`optimize("O2")`) removed the compiler's ability to inline `nn::fmul`/`nn::scale`/`activate` into `forward_layer`/`TrainBatch`/`AccumulateGradients`. On hardware this measured as a pinned-vs-naive session time of 76.08s vs 44.34s (1.72x), unrelated to memory-bank placement or cross-core bus contention -- it reproduced identically with the other core dormant (76.08s) or flooding SRAM (76.10s). The per-weight gradient loop went from 10 inlined instructions/step to 26, with two out-of-line calls (`nn::fmul` entered twice per step across 6,464 steps × 3,823 samples × 10 epochs ≈ 494M calls/session). Fix: `MEML_RUNS_ON_CORE(n)`/`MEML_RUNS_ON_CORE_CODE(n)` in `MemoryDefs.hpp` no longer carry `noinline`/`optimize("O2")`, only the `section(...)` attribute (`used` retained where needed for address-taken entry points and multiply-instantiated templates). Placement now follows the outermost tagged caller: an inlined callee's code lands in whatever bank its caller's section attribute already claims, so hot-path helpers inline away exactly as they do in the naive build while the code that survives as a standalone symbol still lands in the correct bank. `scripts/validate_memory_placement.py` was extended accordingly: it now requires the `MLPOpticalRecognition` entry points (`Initialise`/`Train`/`Predict`) to be present and correctly banked (since most of the hot path no longer leaves its own symbol to check), widened its `smlp::` symbol match to cover const member functions and `for_each_layer`'s lambdas, allowlists the setup-only `get/set_weights_impl`/`get/set_biases_impl` helpers, and adds a benchmarks-mode regression canary that fails if `nn::fmul`/`nn::scale`/`activate` ever reappear as out-of-line symbols. One trade-off to keep in mind for later steps: an *untagged* caller of mlp code now silently pulls that code into its own (wrong) bank with no leftover out-of-line symbol for the validator to catch on inspection alone -- the entry-point check is what still catches this, so every function that enters the MLP from outside must stay tagged with `MEML_MLP_CODE`.
