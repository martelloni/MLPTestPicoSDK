...
- Constructor parameters (replacing the current single-`clock_frequency_hz` constructor):
  ...
  - `runOtherCoreTask` (bool): when `true`, the other core's task calls `RAMFlooder::FillOnce()` every iteration; when `false`, it runs a cheap no-op every iteration instead (see below) so both conditions can be benchmarked from the same code path.
- **Completion signal (the mechanism the old bullet list left unspecified):** add one plain (not `MEML_MLP_DATA`/`MEML_MLP_CODE`-tagged — it is not `smlp::` state and must stay outside `validate_memory_placement.py`'s bank-exclusivity check) `inline static std::atomic<bool> training_done_{false}` member. The MLP-core task sets it to `true` (release ordering) immediately after its **last** training session completes; the other core's task checks it (acquire ordering) at the top of every iteration. This is plain shared SRAM read/written from both cores — RP2350's Cortex-M33 cores have no data cache over SRAM, so a `std::atomic<bool>` with acquire/release is sufficient without extra barriers.
...
- **Bounding the "other core" loop:** `TestBase`'s per-core loop is a fixed iteration count known at construction time ([TestBase.hpp:143-160](tests/TestBase.hpp#L143-L160)); it has no early-exit primitive, and this step must not modify that shared contract. Give the non-MLP core's `TestConfig` iteration count a fixed, generous safety cap (e.g. `training_repeats * epochs_per_session * 50`, documented inline with the sizing rationale) — large enough that `training_done_` is essentially guaranteed to already be `true` well before the cap is reached, after which every further iteration is just an atomic load and a return (cheap, sub-microsecond, does not measurably extend the benchmark's wall-clock time).
...
- Confirm `training_done_` is observed `true` by the flooder core well before its safety-cap iteration count is exhausted (log or assert the actual iteration count consumed vs. the cap, so a too-small cap is caught rather than silently truncating the flood).
